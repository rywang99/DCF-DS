...
def write_transcript_to_stm(
    out_dir, attributed_segments_df: pd.DataFrame, tn, session_id: str, filename
):
    """
    ...
    if "session_id" in attributed_segments_df:
        assert (
            attributed_segments_df.session_id.nunique() <= 1
        ), "no cross-session information is permitted"
    ...
    with filepath.open("w", encoding="utf-8") as f:
        ...
        for entry in range(len(attributed_segments_df)):
            ...
            f.write(
                f"{session_id} {channel} {stream_id} {start_time} {end_time} {text}\n"
            )
...
def calc_wer(
    out_dir: str,
    tcp_wer_hyp_stm: str,
    tcorc_wer_hyp_stm: str,
    session_id: str,
    gt_utt_df: pd.DataFrame,
    tn,
    collar: float,
    ...
    ref_stm_path = write_transcript_to_stm(
        out_dir, df, tn, session_id, filename="ref.stm"
    )

    stm_res = pd.Series(
        {
            "session_id": session_id,
            "tcp_wer_hyp_stm": tcp_wer_hyp_stm,
            "tcorc_wer_hyp_stm": tcorc_wer_hyp_stm,
            "ref_stm": ref_stm_path,
        }
    )
    ...
    def save_wer_visualization(session: pd.Series):
        ...
        av = AlignmentVisualization(
            ref[meeting_name], hyp[meeting_name], alignment="tcp"
        )
    ...
    def calc_session_tcp_wer(session: pd.Series):
        os.system(
            f"{sys.executable} -m meeteval.wer tcpwer "
        ...
        with open(
            os.path.splitext(session.tcp_wer_hyp_stm)[0] + "_tcpwer.json", "r"
        ) as read_file:
            ...
            return pd.Series({("tcp_" + key): data[key] for key in keys}).rename(
                {"tcp_error_rate": "tcp_wer"}
            )

    def calc_session_tcorc_wer(session: pd.Series):
        os.system(
            f"{sys.executable} -m meeteval.wer tcorcwer "
        ...
        with open(
            os.path.splitext(session.tcorc_wer_hyp_stm)[0] + "_tcorcwer.json", "r"
        ) as read_file:
            data = json.load(read_file)
            keys = [
                "error_rate",
            ...
            return pd.Series({("tcorc_" + key): data[key] for key in keys}).rename(
                {"tcorc_error_rate": "tcorc_wer"}
            )
    ...
    _LOG.info(
        f"tcp_wer = {session_res.tcp_wer:.4f}, tcorc_wer = {session_res.tcorc_wer:.4f} "
