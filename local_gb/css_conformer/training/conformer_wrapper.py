...
from css_conformer.css_with_conformer.executor.executor import Executor
from css_conformer.css_with_conformer.nnet.conformer import ConformerCSS
import torch as th
...
class NnetCfg:
    ...
    num_nois: int = 0  # CSS_with_Conformer had 2 noise masks. NOTSOFAR has 1.
...
class ConformerCssWrapper(nn.Module):
    ...
    def separate(self, stft: th.Tensor):
        ...
        # pdb.set_trace()
        res = self.executor({"mix": None, 'mag': stft.abs(), 'pha': stft.angle()}) # 得到四路输出，其中三路表示固定三个说话人的mask，另一个是噪音的mask                                                                   
                                                                                    # mask 维度为 1 * 257 * 186
