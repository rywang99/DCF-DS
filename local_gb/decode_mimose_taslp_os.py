...
def main(args):
    ...
    # nnet_blind = MULTI_MAM_SE_S2S_model_fix_blind(config_train)
    nnet_fix = MULTI_MAM_SE_S2S_model_fix(config_train2)
    # pdb.set_trace()
    try:
    ...
    utils_s2s.load_checkpoint_join_training_init_mask_decode(nnet_fix, None, './SC_DCF_DS.model')
    ...
    nnet_fix = torch.nn.DataParallel(nnet_fix, device_ids = gpus).cuda()
    ...
    spk_mask_numpy = './data/MTG1_eval_sc/MULTI_MAM_SE_S2S_model.model10_MTG_eval_sc_sq_0715_fusion'
    
    # label_init = RTTM_to_Speaker_Mask(args.init_rttm,  max_speaker=args.max_speaker, frame_per_second=args.frame_per_second)
    ...
    for session in tqdm.tqdm(file_list.keys()):
        ...
        spk_num_rttm = './data/MTG1_eval_sc/MULTI_MAM_SE_S2S_model.model10_MTG_eval_sc_sq_0715_fusion/rttm_th0.50_pp_split'
        ...
        if speaker_list != speaker_list1:
            ...
            if len(speaker_list) - len(speaker_list1) == 0:
                ...
                list_out.append([session, speaker_list, speaker_list1, len(speaker_list) - len(speaker_list1)])
                # list_out.append([session, list(new_dict1.keys()), speaker_list1, len(speaker_list) - len(speaker_list1)])
                # (new_dict1['0'] - embedding[session]['0']).sum()
                # (new_dict2['0'] - label_init.frame_label[real_session]['0']).sum()
                embedding[session] = new_dict1
        ...
        # continue
        # pdb.set_trace()
        # wav_fea, feature, intervals, total_frame, wav_stft_fea = load_single_channel_feature_fbank_mcwav(file_list[session], args.max_utt_durance, args.hop_len, wav_fea_all_stft)
        ...
        with torch.no_grad():
            for m in feature:
                ...
                # if cur_spk_mask1.shape[0] == 4:
                    
                    # pdb.set_trace()
                # batch_spk_mask_list.append()
                ...
                if (i == args.batch_size) or (len(feature) == cur_utt): #攒够一个batch
                    ...
                    for ch in range(1):
                        ...
                        dia_ypreds1, dia_ypreds2, mask1, noise_mask, css_model = nnet_fix(wav_data[:,ch:ch+1,:], spk_masks_reshape, wav=1)
                        dia_ypreds1, dia_ypreds2, ypreds, noise_mask, css_model = nnet(wav_data[:,ch:ch+1,:], mask1, wav=1)
                        ypreds1 = ypreds
