...
from css_conformer_noise_jointrain_800_stage2_sc.css_with_conformer.executor.executor import Executor
from css_conformer_noise_jointrain_800_stage2_sc.css_with_conformer.nnet.conformer import ConformerCSS
import torch as th
...
class NnetCfg:
    ...
    in_features: int = 1285 
    num_nois: int = 1  # CSS_with_Conformer had 2 noise masks. NOTSOFAR has 1.
    num_spks: int = 4  # CSS_with_Conformer had 2 spk masks. NOTSOFAR has 3.
...
class ConformerCssWrapper(nn.Module):
    ...
    def forward(self, mix: th.Tensor, mask: th.Tensor):
        ...
        stft = self.stft(mix) # B, F, T, C
        # pdb.set_trace()
        res = self.separate(stft, mask)
        return res

    def separate(self, stft: th.Tensor, mask: th.Tensor):
        ...
        # pdb.set_trace()
        res = self.executor({"mix": None, 'mag': stft.abs(), 'pha': stft.angle(), 'mask': mask}) # 得到四路输出，其中三路表示固定三个说话人的mask，另一个是噪音的mask                                                                   
                                                                                    # mask 维度为 1 * 257 * 186
    ...
    def stft(self, s: th.Tensor):
        ...
        stft_cplx = th.polar(mag.to(th.float32), phase.to(th.float32))  # -> [Batch, Mics, F, T]
