...
def plot_wav(label_data):
    ...
    for i in range(3):
        ...
        if i == 0:
            plt.colorbar()
        # plt.colorbar()
        plt.savefig(f'./sample/mask_label_inmodel{i}.png')
        #pdb.set_trace()
...
def init_kernel(frame_len,
    ...
    if window == "sqrt_hann" and normalize:
        ...
        S = 1
    # F x N/2+1 x 2
    # NOTSOFAR (the following was replaced with the code below): K = torch.fft.rfft(th.eye(N) / S, 1)[:frame_len]
    # Note that torch 1.7.1 seems to ignore the second argument of torch.rfft().
    K = th.view_as_real(th.fft.rfft(th.eye(N) / S))[:frame_len]
    # 2 x N/2+1 x F
...
class STFTBase(nn.Module):
    ...
    def __init__(self,
        ...
        # this is a fix introduced by NOTSOFAR. conjugate used to be True for window == "hann".
...
class IPDFeature(nn.Module):
    ...
    def forward(self, p):
        ...
        if p.dim() == 3:
            p = p.unsqueeze(0)
        N, _, _, T = p.shape
        pha_dif = p[:, self.index_l] - p[:, self.index_r] # 计算其它6个通道与0通道的差值
...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, mix=None, doa=None, ref_channel=0, mag=None, pha=None, mask=None):
        """
        args
            mix: N x C x S (multi-channel) or N x S (single channel)
        ...
        if mag is None:
            self._check_args(mix, doa)
        # mag & pha: N x C x F x T or N x F x T
        mag, pha, f = self.compute_spectra(mix, mag, pha) # f为归一化的幅度谱
        # feature = [f]
        # mask_repeat = mask.unsqueeze(dim=2).repeat(1, 1, 257, 1)
        mask_repeat = mask.permute(0,3,1,2)
        # pdb.set_trace()
        f_unseq = f.unsqueeze(dim=1)
        mag_mask_all = th.cat([f_unseq, mask_repeat], dim = 1)
        B_num, _, Fre_num, Frame_num = mag_mask_all.shape
        mag_mask_all_cat = mag_mask_all.contiguous().view(B_num, -1, Frame_num)
        feature = [mag_mask_all_cat]
        # plot_wav(mask.cpu())
        # import matplotlib.pyplot as plt
        # for i in range(4):
            # plt.imshow(mask_repeat[1,i,:,:].cpu(), cmap='binary' , interpolation = 'none')
            # if i == 0:
                # plt.colorbar()
            # plt.savefig(f'./sample/mask_label{i}_inmodel.png')
        # pdb.set_trace()
        if self.has_spatial:
